...
\pagestyle{fancy}
\fancyhf{}
\lhead{Erd\H{o}s \#287 -- V16.6 Theory Manual R7}
\rhead{30 August 2026}
...
\large Clean Public Review + Self-Contained Gate Theory Manual\\[0.3ex]\normalsize Version 16.6 -- Public Prepublication Candidate R7 (not yet published)}
...
\textbf{Status.} Erd\H{o}s Problem \#287 remains open.  The unconditional public Lean
theorem excludes every exact counterexample with maximum denominator at most
$4\times10^9$, and the large-$M$ conclusion remains compiled from the explicit
\texttt{WindowPairSupply} interface.  Neither statement is altered by the later analytic
programme.

The fixed-certificate Balanced7 cell has now passed the completed hostile analytic audit
at the \emph{research/paper} level: the SmallQ conductor-split $3+4$ large-sieve route,
general-modulus SmallR estimate, physical hard-$\theta$ short-$t$/Shiu estimate and exact
full-$q$ ownership/reassembly all pass under the literal SP-2 source used in the audit.
This does \emph{not} make the analytic closure Lean-proved, and the manuscript-repository
does not presently contain the later supplied $8213$-job post-Balanced7 safe bank.
Accordingly the exact SP-2-to-seven-box source seal remains a separate provenance
obligation, and effective Balanced7 remains open because the low-conductor input uses
classical ineffective technology.

The controlling main line has therefore moved downstream to the uniform $k=0$ SP-2
reassembly.  The latest exact structural reduction compresses the signed cross-level
shared-gcd Gram into a one-level M\"obius operator.  The current first exact main-line
analytic residual is
\[
\boxed{\texttt{287-K0-SP2-DET1-SHAREDGCD-ONELEVEL-MOBIUS-GRAM45}}.
\]
The earlier large-shared-$g_0$ and near-frequency \emph{global closure} claims are
retracted; their fixed-pair/local gains remain valid.  The product-modulus sign compression,
fixed-$D$ frequency rigidity, CRT reciprocal normal form and shared-gcd Gram-as-square are
latest research-exact reductions whose formal banking is not yet verified in this public
manuscript repository.

Uniform $k=0$, FCL, $N_2$, Gate~2, effectivity, \texttt{WindowPairSupply}, and
Erd\H{o}s \#287 remain open/not activated as stated below.
\end{minipage}}
\end{center}
...
at most $4\times10^9$.  The remaining global range is isolated by the machine-checked
conditional interface \texttt{WindowPairSupply}; no eventual supply theorem is proved.

The fixed-certificate smooth-parity source uses
\[
G_*(m;n)=\mathbf1_{m\le n^{\gamma_*}}\mu(m_1)g_*(\mathbf v(m_2;n)),
\qquad
H_*(n)=\sum_{m\mid n}G_*(m;n).
\]
On the balanced seven-prime SP-2 cell, $H_*(n)=-20$.  The literal affine factorization is
\[
\Lambda(2P+s)=\sum_{qr=2P+s}\mu(q)\log r,
\qquad s=\pm1,
\]
so the physical logarithmic source cost is $C_{\rm ext}=1$.  The exact full-$q$ principal
Euler packet, SmallQ/SmallR/Hard partition, conductor-split $3+4$ multiplicative large
sieve, general-modulus SmallR estimate, physical hard-range short-$t$ upper sieve, Shiu
divisor average and exact owner reassembly survive the completed hostile audit.  At the
current research/paper level this closes the Balanced7 asymptotic packet, conditional only
on source-sealing the literal SP-2 seven-box adapter.  The low-conductor use of classical
Siegel--Walfisz remains ineffective, so no computable threshold is claimed.

The main analytic frontier has therefore moved to uniform $k=0$.  In the determinant-one
cross-level family write
\[
g_1=g_0r_1,\qquad g_2=g_0r_2,\qquad n=r_1r_2,
\]
with $(r_1,r_2)=1$ and $(g_0,n)=1$.  The squarefree source gives the exact sign compression
\[
\mu(g_1)\mu(g_2)=\mu(n),\qquad [g_1,g_2]=g_0n.
\]
For fixed determinant
\[
D=t_1r_2-t_2r_1
\]
the primitive frequencies are rigid modulo $r_1,r_2$ and
\[
\frac{t_1}{g_1}-\frac{t_2}{g_2}=\frac{D}{g_0n}.
\]
CRT combines the remaining reciprocal phases into an additive-reciprocal kernel modulo
$n$.  More decisively, the shared-gcd projector has the divisor-lattice M\"obius transform
\[
\lambda_H=\mu*\Omega_H,
\]
and exact inversion reorganizes the signed cross-level Gram as
\[
Q_H=
...
\right|^2 .
\]
Thus the apparent two-level $G^2/H$ signed-pair problem becomes a one-level signed M\"obius
operator with harmonic shared-gcd mass.  This is an exact structural reduction, not yet an
analytic estimate.

A coefficient-blind energy bound closes a strict exponent subpolytope only provisionally;
hostile promotion audit remains required.  Fully completing the primitive-frequency
variable is nonclosing because the generic Ramanujan local sign cancels the remaining
M\"obius sign, producing a $\zeta(1+w)/\zeta(2+2w)$ pole rather than a $1/\zeta$ zero.
Accordingly the first exact main-line analytic residual is
\[
\boxed{\texttt{287-K0-SP2-DET1-SHAREDGCD-ONELEVEL-MOBIUS-GRAM45}},
\]
which must exploit the one-level M\"obius sign before complete primitive-frequency
Ramanujan reassembly neutralizes it.  Uniform $k=0$, FCL, $N_2$, Gate~2, effectivity,
eventual \texttt{WindowPairSupply}, and Erd\H{o}s \#287 remain open.
\end{abstract}
...
Strongest unconditional theorem; unchanged.\\
\texttt{WindowPairSupply} compiler &
\status{PUBLIC LEAN-CHECKED / CONDITIONAL} &
Effective eventual supply plus the finite theorem implies the public statement.\\
V15--V19 public formal banks &
\status{PUBLIC-MAIN SYNCHRONIZED} &
Aggregate public imports exist; supplied reports record successful $8118$, $8123$,
$8131$, $8135$, $8140$-job builds, respectively.\\
Later post-Balanced7 $8213$-job bank &
\status{SUPPLIED SAFE BANK / PUBLIC SYNC PENDING} &
Reported append-only build, but the named later modules are not present in the current
public manuscript repository.\\
Balanced7 full-$q$ analytic packet &
\status{HOSTILE-AUDITED RESEARCH/PAPER PASS} &
SmallQ, SmallR, Hard and exact owner reassembly pass under the literal SP-2 source used in
the audit; not a Lean analytic theorem.\\
Balanced7 exact SP-2 source seal &
\status{SOURCE-OPEN / PUBLIC SEAL NOT VERIFIED} &
The current public repository does not expose a theorem sealing the complete SP-2
seven-box adapter targeted by the later safe bank.\\
Balanced7 effectivity &
\status{OPEN} &
Classical low-conductor Siegel--Walfisz does not give a computable threshold.\\
\parbox[t]{0.29\textwidth}{\raggedright\ttfamily 287-K0-SP2-UNIFORM-\\FRAGMENTATION-REASSEMBLY45} &
\status{OPEN / CURRENT MAIN PARENT} &
Uniform $k=0$ source theorem after Balanced7.\\
Large-shared-$g_0$ / near-frequency local routers &
\status{LOCAL PASS; GLOBAL CLOSURE RETRACTED} &
Fixed-pair reciprocal cancellation survives, but restoring full $G^2/H$ cross-level
multiplicity invalidates the old global closure inference.\\
Short-lift Euler collapse &
\status{PASS AFTER UNIFORMITY REPAIR} &
Uses the $t$-independent zero-free contour; Lean analytic input remains uninhabited.\\
Product-modulus sign compression / fixed-$D$ rigidity / CRT normal form &
\status{RESEARCH-EXACT / FORMAL BANKING PENDING} &
Exact algebraic reductions; not yet verified in the public manuscript repository.\\
Shared-gcd Gram-as-square &
\status{RESEARCH-EXACT PASS / FORMAL BANKING PENDING} &
Compresses the cross-level signed Gram into a one-level M\"obius operator.\\
One-level coefficient-blind energy &
\status{PROVISIONAL SUBPOLYTOPE CLOSURE} &
Fixed-power saving on a strict exponent subpolytope; hostile promotion audit still required.\\
Complete primitive-$t$ Ramanujan route &
\status{NONCLOSING / REPRESENTATION LOOP} &
Ramanujan local signs neutralize the M\"obius sign; ordinary Euler $1/\zeta$ cancellation is
not available.\\
\parbox[t]{0.29\textwidth}{\raggedright\ttfamily
287-K0-SP2-DET1-SHAREDGCD-\\ONELEVEL-MOBIUS-GRAM45} &
\status{FIRST EXACT MAIN-LINE ANALYTIC RESIDUAL / OPEN} &
Exploit the one-level signed M\"obius operator while retaining primitive frequency,
reciprocal source, friable average and exact shared-gcd projector.\\
FCL / $N_2$ / Gate~2 &
\status{NOT REACHED / NOT ACTIVATED} &
Downstream only after uniform $k=0$ and remaining fixed-certificate packet census.\\
Effectivity / eventual \texttt{WindowPairSupply} &
\status{OPEN} &
A separate effective low-conductor/exceptional PNT dictionary remains required.\\
Erd\H{o}s Problem \#287 &
\status{OPEN} &
No unconditional solution claim.\\
\bottomrule
\end{tabularx}
\end{center}

\paragraph{Status vocabulary.}
\status{PUBLIC LEAN-CHECKED} and \status{PUBLIC-MAIN SYNCHRONIZED} refer to theorem/source
objects actually present in the public formal project.  \status{SUPPLIED SAFE BANK /
PUBLIC SYNC PENDING} records a supplied successful replay whose later modules are not yet
visible in this manuscript repository.  \status{RESEARCH-EXACT} means an exact mathematical
reduction has passed the present research audit but is not claimed public-Lean checked.
\status{HOSTILE-AUDITED RESEARCH/PAPER PASS} is an analytic research verdict using external
theorems and is not a Lean analytic proof.  \status{SOURCE-OPEN} is a missing literal source
identification.  \status{OPEN} is an unproved analytic or effective theorem.
\status{RETRACTED} applies to the stated global closure inference, not automatically to
every local identity developed along that route.
...
Balanced7 does not complete the $k=0$ fixed-certificate source.  The next parent is
\[
...
\section{Current \#287 frontier after Balanced7}
\label{sec:fixed-degree-seven-ledger}

The fixed-degree-seven Balanced7 packet is now an audited input to the next source layer,
not the first main-line analytic residual.  Its status is:
\[
\boxed{\begin{gathered}
...
\]
Within its determinant-one cross-level sector the latest exact reductions are:
\begin{enumerate}[leftmargin=1.7em]
...
The controlling one-level object is
\[
\mathcal G_{H,d}
=
\sum_{a\asymp A}\rho(a)
...
\right|^2,
\]
and the current first exact main-line target is
\[
\boxed{
\left|
\sum_d\frac{\lambda_H(d)}{d^2}\mathcal G_{H,d}
...
firewalls, but they do not control R7:
\begin{itemize}[leftmargin=1.6em]
...
\subsection*{Supplied later safe bank}
A later supplied report records an $8213$-job successful append-only build with
...
Those modules are not currently present in this public manuscript repository.  Their R7
status is \status{SUPPLIED LEAN SAFE BANK / REPOSITORY SYNC PENDING}.
...
\subsection*{Latest research-exact layer, formal banking pending}
The product-modulus M\"obius-sign compression, fixed-$D$ frequency rigidity, CRT
$n$-reciprocal normal form and shared-gcd Gram-as-square are currently research-exact
mathematics.  R7 does not call them Lean-verified until a corresponding successful formal
bank is supplied and synchronized.
...
zero-free-region analytic statement.  The first one-level M\"obius Gram remains open.
